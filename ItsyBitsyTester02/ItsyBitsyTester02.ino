...
#define TIMEOUT_MS     (2000) // Number of milliseconds before a record is invalidated in the list
...
if (VERBOSE_OUTPUT | return_of_the_mac){
  uint8_t len = 0;
  uint8_t buffer[32];
...
//   }
  ...
  if(report->rssi < minimum) {
    minimum = report->rssi;
  }
  if(report->rssi > maximum) {
    maximum = report->rssi;
  }

  Serial.println(180 * (( abs(report->rssi) - abs(minimum) ) / ( abs(maximum) - abs(minimum) )));
